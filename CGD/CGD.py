...
class CGD(torch.nn.Module):
    ...
    ***The input data X could be a torch tensor OR a dictionary of torch tensors. ***
    ...
    *       - indicates other dimensions, e.g., subjects or conditions, across which N and P are equal.
    ...
    def __init__(self, num_comp, X, Xtilde=None, model="SPCA", G_idx=None, lambda1=None, lambda2=None, init=None):
        ...
        # if X is not a dictionary of torch tensors, we turn it into one
        try:
            self.keys = X.keys()
            num_groups = len(X)  # infer number of modalities from number of entries in dictionary X
        ...
        if model == "SPCA":
            self.Xsqnorm = torch.zeros(num_groups)
            self.XtXtilde = torch.zeros((num_groups, *other_dims, P, torch.sum(G_idx)))
        ...
        self.S_size = [num_groups, *other_dims, num_comp, P]
        ...
        if self.model == "AA" or self.model == "DAA":
            ...
            if init is None:
                self.G = torch.nn.Parameter(self.softmaxG(torch.rand((torch.sum(G_idx).int(), num_comp))))
                self.S = torch.nn.Parameter(self.softmaxS(torch.rand(self.S_size)))
            else:
                self.G = init["G"].clone()
                self.S = init["S"].clone()
            ...
            if init is None:
                self.Bp = torch.nn.Parameter(torch.rand((torch.sum(G_idx).int(), num_comp)))
                self.Bn = torch.nn.Parameter(torch.rand((torch.sum(G_idx).int(), num_comp)))
            else:
    ...
    def forwardAA(self, G_soft, S_soft):
        # loss function for archetypal analysis
        loss = 0
        for m, key in enumerate(self.keys):
    ...
    def forward(self):
        # compute the loss depending on the model
        if self.model == "AA" or self.model == "DAA":
            ...
            loss += self.lambda1 * torch.sum((Bpsoft + Bnsoft)) # L1 regularization term
            loss += self.lambda2 * torch.sum((Bpsoft**2 + Bnsoft**2)) # L2 regularization term
    ...
    def eval_model(self, Xtrain, Xtest, Xtraintilde=None, G_idx=None):
        ...
        with torch.no_grad():
            ...
            if self.model == "AA" or self.model == "DAA":
                ...
                XtXtilde = torch.zeros((num_modalities, *other_dims, P, torch.sum(G_idx)))
                for m, key in enumerate(self.keys):
    ...
    def get_model_params(self):
        # return the model parameters after estimation
        with torch.no_grad():
            if self.model == "AA" or self.model == "DAA":
